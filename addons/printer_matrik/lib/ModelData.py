...
class ModelData:
    def __init__(self, flectra_instance, model_name, model_line_name, treeview, search_entry, page_label, back_button, forward_button, print_button, status_label, default_condition = -1):
        self.flectra = flectra_instance
    ...
    def count_page(self, conditions):
        num = self.flectra.env[self.model_name].search_count(conditions)
    ...
    def prepare_saleorder_data(self, fields, condition_lists):
        ...
        order = fields[0] + ' desc'
        records = self.flectra.env[self.model_name].search_read(condition_lists, fields=fields, limit=GeneralData.page_limit, offset=offset, order=order, context={'lang': "id_ID"})
        data_list = []
        for record in records:
            data_list.append([
                Formatter.format_flectra_field(Formatter.format_date(record.get(GeneralData.saleorder_fields[0]), treeview_format=True)),
                Formatter.format_flectra_field(record.get(GeneralData.saleorder_fields[1])),
                Formatter.format_flectra_field(record.get(GeneralData.saleorder_fields[2])),
                Formatter.format_flectra_field(record.get(GeneralData.saleorder_fields[3])),
                Formatter.format_flectra_field(record.get(GeneralData.saleorder_fields[4])),
                Formatter.format_flectra_field(str(record.get(GeneralData.saleorder_fields[5]))),
                ])
        return data_list
    ...
    def prepare_stockpicking_data(self, fields, condition_lists):
        ...
        for record in records:
            data_list.append([
                Formatter.format_flectra_field(Formatter.format_date(record.get(GeneralData.stockpicking_fields[0]), treeview_format=True)),
                Formatter.format_flectra_field(record.get(GeneralData.stockpicking_fields[1])),
                Formatter.format_flectra_field(record.get(GeneralData.stockpicking_fields[2])),
                Formatter.format_flectra_field(record.get(GeneralData.stockpicking_fields[3])),
                Formatter.format_flectra_field(Formatter.map_stockpicking_status(record.get(GeneralData.stockpicking_fields[4]))),
                Formatter.format_flectra_field(str(record.get(GeneralData.stockpicking_fields[5]))),
                ])
        return data_list
    ...
    def prepare_invoice_data(self, fields, condition_lists):
        ...
        for record in records:
            data_list.append([
                Formatter.format_flectra_field(Formatter.format_date(record.get(GeneralData.invoice_fields[0]), date_only=True, treeview_format=True)),
                Formatter.format_flectra_field(Formatter.format_date(record.get(GeneralData.invoice_fields[1]), date_only=True, treeview_format=True)),
                Formatter.format_flectra_field(record.get(GeneralData.invoice_fields[2])),
                Formatter.format_flectra_field(record.get(GeneralData.invoice_fields[3])),
                Formatter.format_flectra_field(record.get(GeneralData.invoice_fields[4])),
                Formatter.format_flectra_field(str(record.get(GeneralData.invoice_fields[5]))),
                ])
        return data_list
    ...
    def prepare_saleorder_document(self, ids, fields, line_fields):
        saleorder_data = self.flectra.env[self.model_name].read(ids, fields=fields)[0]
        partner_data = self.flectra.env['res.partner'].read(saleorder_data.get('partner_id')[0], fields=GeneralData.partner_fields)[0]
        for field in fields:
            if field != 'order_line':
                saleorder_data[field] = Formatter.format_flectra_field(saleorder_data.get(field))
            if 'date' in field:
                saleorder_data[field] = Formatter.format_date(saleorder_data.get(field))
        for field in GeneralData.partner_fields:
            partner_data[field] = Formatter.format_flectra_field(partner_data.get(field))
        saleorder_line_data = self.flectra.env[self.model_line_name].read(saleorder_data.get('order_line'), fields=line_fields)
        for data in saleorder_line_data:
            for field in GeneralData.saleorder_line_fields:
                data[field] = Formatter.format_flectra_field(data.get(field))
    ...
    def prepare_stockpicking_document(self, ids, fields, line_fields):
        stockpicking_data = self.flectra.env[self.model_name].read(ids, fields=fields)[0]
        partner_data = self.flectra.env['res.partner'].read(stockpicking_data.get('partner_id')[0], fields=GeneralData.partner_fields)[0]
        for field in fields:
            if field != 'move_lines':
                stockpicking_data[field] = Formatter.format_flectra_field(stockpicking_data.get(field))
            if 'date' in field:
                stockpicking_data[field] = Formatter.format_date(stockpicking_data.get(field))
        for field in GeneralData.partner_fields:
            partner_data[field] = Formatter.format_flectra_field(partner_data.get(field))
        stockpicking_line_data = self.flectra.env[self.model_line_name].read(stockpicking_data.get('move_lines'), fields=line_fields)
    ...
    def prepare_invoice_document(self, ids, fields, line_fields):
        invoice_data = self.flectra.env[self.model_name].read(ids, fields=fields)[0]
        partner_data = self.flectra.env['res.partner'].read(invoice_data.get('partner_id')[0], fields=GeneralData.partner_fields)[0]
        for field in GeneralData.invoice_detail_fields:
            if field != 'invoice_line_ids':
                invoice_data[field] = Formatter.format_flectra_field(invoice_data.get(field))
            if 'date' in field:
                invoice_data[field] = Formatter.format_date(invoice_data.get(field), date_only=True)
        for field in GeneralData.partner_fields:
            partner_data[field] = Formatter.format_flectra_field(partner_data.get(field))
        invoice_line_data = self.flectra.env[self.model_line_name].read(invoice_data.get('invoice_line_ids'), fields=line_fields)
