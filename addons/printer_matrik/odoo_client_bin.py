...
import os, flectrarpc, ssl, urllib.request, configparser
...
class MainPage:
    ...
    def on_okay_button_clicked(self, button, data=None):
        ...
        try:
            self.flectra.login(GeneralData.db_name, username, password)
            self.initiate_window()
        except flectrarpc.error.RPCError as exc:
    ...
    def __init__(self):
        ...
        self.flectra = self.prepare_flectra_instance()
    ...
    def prepare_flectra_instance(self):
        ssl_context = ssl.create_default_context()
        ssl_context.check_hostname = False
        ssl_context.verify_mode = ssl.CERT_NONE

        opener = urllib.request.build_opener(urllib.request.HTTPSHandler(context=ssl_context))
        return flectrarpc.flectra(GeneralData.url, protocol='jsonrpc+ssl', port=GeneralData.port, opener=opener)
    ...
    def initiate_window(self):
        ...
        self.status_label.set_text('Log masuk sebagai : ' + self.flectra.env.user.name)

    def check_groups(self):
        groups = self.flectra.env.user.groups_id
        parsed_groups = []
        for group in groups:
            parsed_groups.append((group.full_name).split('/')[0])
        if any('Penjualan' in group for group in parsed_groups) and self.flectra.env['sale.order'].check_access_rights:
            UserData.give_sales_access()
        if any('Persediaan' in group for group in parsed_groups) and self.flectra.env['stock.picking'].check_access_rights:
            UserData.give_stock_access()
        if any('Akuntansi' in group for group in parsed_groups) or any('Penagihan' in group for group in parsed_groups) and self.flectra.env['account.invoice'].check_access_rights:
